...
public class Tools {
    ...
    public static void showDialogAbout(Activity activity) {
        Dialog dialog = new DialogUtils(activity).buildDialogInfo("About", "Content", "OK", R.drawable.img_about, new CallbackDialog() {
            @Override
...
//    }
    ...
    public static String getVersionName(Context ctx) {
        try {
            PackageManager manager = ctx.getPackageManager();
            PackageInfo info = manager.getPackageInfo(ctx.getPackageName(), 0);
//            return ctx.getString("Version") + " " + info.versionName;
            return "";
        } catch (PackageManager.NameNotFoundException e) {
//            return ctx.getString("Version");
            return "";
        }
    }
